      ...
      {isSidebarExpanded ? (
        <Link
          ...
          className={`h-full relative flex items-center whitespace-nowrap rounded-md ${
            active
              ? "font-base text-sm bg-green-200 shadow-sm text-green-700 dark:bg-green-800 dark:text-white"
              : "hover:bg-green-100 hover:text-green-700 text-green-500 dark:text-green-400 dark:hover:bg-green-800 dark:hover:text-white"
          }`}
        >
          <div className="relative font-base text-sm py-1.5 px-2 flex flex-row items-center space-x-2 rounded-md duration-100">
            <span className="text-green-600">{icon}</span> {/* Icon color */}
            <span>{label}</span>
          </div>
        </Link>
      ) : (
